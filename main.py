...
class PlaneGame:
    ...
    def __create_NARS(self, type_):
        if type_.lower() == "opennars":
            self.nars = OpenNars()
            self.remaining_babble_times = 200
        elif type_.lower() == "ona":
            self.nars = ONA()
    ...
    def __event_handler(self):
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                self.nars.update(self.hero, self.enemy_group)
                # use objects' positions to update NARS' sensors
    ...
    def __display_text(self):
        ...
        surface_babbling = self.font.render("Babbling: %d" %
                                            self.remaining_babble_times, True, [235, 235, 20])
        surface_nars_type = self.font.render(self.nars_type, True, [235, 235, 20])
        surface_version = self.font.render(__version__, True, [235, 235, 20])
